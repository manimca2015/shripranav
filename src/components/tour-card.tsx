...
export function TourCard({ tour }: TourCardProps) {
  ...
  return (
    <>
      <Card className="relative rounded-3xl overflow-hidden shadow-xl shadow-slate-200/50 card-hover border border-slate-100 flex flex-col h-full group/card bg-white">
        ...
        <div className="p-6 flex flex-col flex-grow">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <h3 className="text-xl font-bold font-headline text-primary leading-tight transition-colors group-hover/card:text-accent">
              <Link href={`/tours/${tour.id}`} className="after:absolute after:inset-0">
                {tour.title}
              </Link>
            </h3>
          </div>
        ...
        <div className="px-6 pb-6 flex gap-3 relative z-20">
          {tour.brochureUrl && (
            <Button
              variant="outline"
              size="icon"
